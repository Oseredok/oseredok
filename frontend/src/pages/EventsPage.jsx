...
export default function EventsPage({ user, onNavigateToEvent, onViewChange }) {
  ...
  const filteredEvents = allEvents.filter((e) => {
    const isPast = e.start_datetime && new Date(e.start_datetime) < now;
    ...
    <div>
      ...
      <div style={{ marginBottom: 28, display: "flex", flexDirection: "column", gap: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          ...
          />
          <div style={{ display: "flex", borderRadius: radius.md, border: `1.5px solid ${colors.border}`, overflow: "hidden" }}>
            <button
              type="button"
              ...
              style={{
                ...
                background: !showPast ? colors.primary : colors.surface,
                color: !showPast ? colors.white : colors.textSecondary,
              }}
            >
              Майбутні
            </button>
          ...
          {canCreateEvent(user?.role) && (
            <button
              ...
              style={{
                ...
                cursor: "pointer", boxShadow: "0 2px 8px rgba(0,82,204,0.25)",
        ...
        {categories.length > 1 && (
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {categories.map((cat) => {
              ...
              const color = cat === "Всі" ? colors.primary : categoryColors[cat] || categoryColors.default;
              return (
                <button
                  key={cat}
                  type="button"
                  ...
                  style={{
                    padding: "7px 16px", borderRadius: radius.pill, fontSize: 12,
      ...
      {!loading && filteredEvents.length > 0 && (
        <div style={{ marginBottom: 20, fontSize: 13, color: colors.textMuted, fontWeight: 500, fontFamily: fonts.body }}>
      ...
      {!loading && error && (
        <div style={{ textAlign: "center", padding: "80px 20px" }}>
          <h3 style={{ fontSize: 18, fontWeight: 700, color: colors.text, marginBottom: 8, fontFamily: fonts.heading }}>
      ...
      {!loading && !error && hasLoaded && filteredEvents.length === 0 && (
        <div style={{ textAlign: "center", padding: "80px 20px" }}>
          <h3 style={{ fontSize: 18, fontWeight: 700, color: colors.text, marginBottom: 8, fontFamily: fonts.heading }}>
            {showPast ? "Минулих подій не знайдено" : "Майбутніх подій не знайдено"}
          </h3>
          ...
          <button
            ...
            style={{
              padding: "10px 24px", borderRadius: radius.pill, fontSize: 13, fontWeight: 700,
              border: `1.5px solid ${colors.primary}`, background: colors.surface,
              color: colors.primary, cursor: "pointer", fontFamily: fonts.body,
            }}
          >
            Скинути фільтри
          </button>
